...
{
    SubscribeToEvent(E_BEGINFRAME, CROSS_HANDLER(MasterServerClient, HandleBeginFrame));
}
...
void MasterServerClient::RegisterServerWithMaster(const String &name)
{
    ...
    Cross::Network* network = GetSubsystem<Network>();
...
void MasterServerClient::SendMessageToMasterServer(const String& msg)
{
    ...
    {
        CROSS_LOGERROR("No master server connection. Cannot send message");
    }
}
...
void MasterServerClient::CheckForNatPunchThroughRequests(float dt)
{
    if (clientIdToPunchThroughSocketMap_.Size() > 0)
    {
        if (timeTillNextPunchThroughAttempt_ <= 0)
        {
                 ...
                 i != clientIdToPunchThroughSocketMap_.End();)
            {
                ...
                CROSS_LOGINFO("Sending packet to client");
        ...
        clientToServerSocket_ != NULL && timeTillNextClientConnectAttempt_ <= 0)
    {
        ...
        {
            CROSS_LOGINFO("Sending packet to server");
        ...
        while (n < totalBytes) {
            ...
            if (readingMasterMessageLength) {
                if (c == ':') {
                    ...
                    CROSS_LOGINFO("Message is " + String(bytesRemainingInMasterServerMessage_) + " long");
                }
                else {
...
void MasterServerClient::SetConnectToMasterState(ConnectToMasterState state)
{
    ...
    {
        CROSS_LOGERROR("Could not connect to master server");
...
void MasterServerClient::SetConnectToGameServerState(ClientConnectToGameServerState state)
{
        ...
        state == GAME_CONNECTING_INTERNAL_IP)
    {
        ...
        CROSS_LOGINFO("Connecting to Game Server on Internal IP: " +
             ...
             state == GAME_CONNECTING_EXTERNAL_IP)
    {
        ...
        CROSS_LOGINFO("Connecting to Game Server on External IP: " +
...
void MasterServerClient::ConnectToGameServerUpdate(float dt)
{
    ...
    if (network->GetServerConnection() && network->GetServerConnection()->IsConnected())
    {
        ...
        {
            CROSS_LOGINFO("Successfully connected using internal IP");
        }
        else if (clientConnectToGameServerState_ == GAME_CONNECTING_EXTERNAL_IP)
        {
            CROSS_LOGINFO("Successfully connected using external IP");
        }
    ...
    if (connectToGameServerSecondsRemaining_ <= 0)
    {
        if (clientConnectToGameServerState_ == GAME_CONNECTING_INTERNAL_IP)
        {
            CROSS_LOGINFO("Unable to connect via internal IP, trying external IP");
...
{
    CROSS_LOGINFO("Got master server message: " + msg);

    rapidjson::Document document;
    if (document.Parse<0>(msg.CString()).HasParseError())
    {
        CROSS_LOGERROR("Could not parse JSON data from string");
        return;
    }
    ...
    else if (cmd == "sendPacketToClient")
    {
        ...
        CROSS_LOGINFO("Got request to send packet to client at "+clientIP+":"+String(clientPort));
                                                          ...
                                                          unsigned short masterPort, const String &serverName)
{
    ...
    if (!network)
    {
        CROSS_LOGERROR("MasterServerClient::StartServerAndRegisterWithMaster - Unable to get Network subsystem");
        return false;
    }
    ...
    if (!rc)
    {
        CROSS_LOGERROR("MasterServerClient::StartServerAndRegisterWithMaster - Unable to start server");
        return false;
    }
